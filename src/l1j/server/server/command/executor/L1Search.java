	...
	public void execute(L1PcInstance pc, String cmdName, String arg) {
			try {
				...
				while (st.hasMoreTokens()) {
					...
					if (itCount == 0 && (tempVar.equals("armor") || 
								tempVar.equals("weapon") || 
				...
				pc.sendPackets(new S_SystemMessage(".find (type[armor,misci,weapon,npc,map]) searchText"));
			}
		}
		   ...
		   if (error  ==  false) {
			   ...
			   while (rs.next()) {
				   ...
				   if (type.equals("misci") || type.equals("weapon") || type.equals("armor")) {
					   bless = rs.getInt(3);
					   pc.sendPackets(new S_SystemMessage(getBlessColor(bless) + "ID: " + str1 + ", " + str2));
				   } else {
	...
	private void find_object(L1PcInstance pc, String name) {
		...
		try {
		   ...
		   while (rs.next()) {
			   ...
			   pc.sendPackets(new S_SystemMessage(String.format("mapid: %s name: %s, startx: %s endx: %s, starty: %s endy: %s", rs.getString(1), rs.getString(2), rs.getString(3), rs.getString(4), rs.getString(5), rs.getString(6))));
			   mapCount++;
		   }
		   ...
		   found = found.length() > 0
			   ? found.substring(0, found.length() - 1) + "." : " Nothing";
		   pc.sendPackets(new S_SystemMessage(found));
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
