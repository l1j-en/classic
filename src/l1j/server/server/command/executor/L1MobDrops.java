...
public class L1MobDrops implements L1CommandExecutor {
	...
	public void execute(L1PcInstance pc, String cmdName, String arg) {
		...
		ResultSet rs = null;
		try {
			final int mobID = Integer.parseInt(arg);
			...
			pc.sendPackets(new S_SystemMessage(theMob.get_name() + "(" + mobID
					+ ") | gfxID: " + gfxID + " | drops:"));
			for (L1Drop drop : DropTable.getInstance().getDrops(mobID)) {
				...
				pc.sendPackets(new S_SystemMessage(blessed + drop.getMin()
						+ "-" + drop.getMax() + " " + drop.getItemid() + " "
						+ item.getName() + " " + chance + "%"));
			}
		} catch (Exception e) {
