...
public class AuctionTimeController implements Runnable {
	...
	private void endAuction(L1AuctionBoard board) {
		...
		if (oldOwnerId != 0 && bidderId != 0) { // Previously, the successful
												// bidder with owner
			...
			if (oldOwnerPc != null) { // The previous owner online
				...
				// Your house was owned by the Adena 1 percent in the final bid
				// price. %n
				// Commission, excluding the remaining 10% percent of the amount
				// of Adena 0% response. %n Thank you. %n%n
			...
			if (bidderPc != null) { // A successful bidder online
				// Congratulations. %n You are joining in the final auction
				// price %0 Adena bid price. %n
				// We like to buy a house that was immediately available. %n
				// Thank you.%n%n
			...
			deleteNote(houseId);
		} else if (oldOwnerId == 0 && bidderId != 0) { // None of the previous
			...
			setHouseInfo(houseId, bidderId);
			deleteNote(houseId);
		} else if (oldOwnerId != 0 && bidderId == 0) { // Previously, the
														// successful bidder
														// without owner
			L1PcInstance oldOwnerPc = (L1PcInstance) L1World.getInstance()
					.findObject(oldOwnerId);
			if (oldOwnerPc != null) { // The previous owner online
				// You did you apply for the auction, the auction period, more
				// than the amount offered to pay for it is expressed, in the
				// end cancelled. %n
				// Therefore, ownership is returned to inform you. %n Thank you.
				// %n%n
				oldOwnerPc.sendPackets(new S_ServerMessage(528));
			}
			deleteNote(houseId);
		} else if (oldOwnerId == 0 && bidderId == 0) { // None of the previous
														// owner without winning
														// bidder
