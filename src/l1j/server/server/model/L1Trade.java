...
import l1j.server.server.hackdetections.LogTradeAddItem;
import l1j.server.server.hackdetections.LogTradeComplete;
		...
		if (l1iteminstance != null && trading_partner != null) {
			if (!l1iteminstance.isEquipped()) {
				...
				trading_partner.sendPackets(new S_TradeAddItem(l1iteminstance,
						itemcount, 1));
				int itemafter = player.getInventory().countItems(l1iteminstance.getItem()
						.getItemId()); 
				LogTradeAddItem ltai = new LogTradeAddItem(); 
				ltai.storeLogTradeAddItem(player, trading_partner, l1iteminstance, 
						itembefore, itemafter, itemcount);
			}
		}
	}
	...
	public void TradeOK(L1PcInstance player) {
		...
		if (trading_partner != null) {
			...
			for (cnt = 0; cnt < player_tradecount; cnt++) {
				...
				player.getTradeWindowInventory().tradeItem(l1iteminstance1, l1iteminstance1.getCount(),
				  ...
				  ltc.storeLogTradeComplete(player, trading_partner, l1iteminstance1, player_tradecount,
						  itembeforeinven, itemafter, player_tradecount);
			}
			for (cnt = 0; cnt < trading_partner_tradecount; cnt++) {
				...
				trading_partner.getTradeWindowInventory().tradeItem(
				 ...
				 LogTradeComplete ltc = new LogTradeComplete(); 
				 ltc.storeLogTradeComplete(trading_partner, player, l1iteminstance2, trading_partner_tradecount, 
