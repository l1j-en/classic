...
public class MpBugTest {
	...
	// Recursively send a packet to every player in game, hoping that the same
	// network
	...
	public void sendTestPackets() {
		for (L1PcInstance pc : L1World.getInstance().getAllPlayers()) {
			...
			// the odds of the last player being the same as the last check
			// increase.
	...
	// If the last player communicated with, plus the last random number
	// generated
	// are the same as the last time this check ran, then more than likely,
	// there is
	// a lockup condition with the socket. synchronized just in case the test
	// thread
