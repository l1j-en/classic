...
public class L1Magic {
	...
	private static final int[] NOT_IN_SAFE_ZONES = new int[] { WEAPON_BREAK,
			...
			RETURN_TO_NATURE, DARK_BLIND, SILENCE, FREEZING_BREATH, RESIST_FEAR,
			GUARD_BRAKE, HORROR_OF_DEATH};
	...
	private int calcProbability(int skillId) {
		...
		if (skillId == ELEMENTAL_FALL_DOWN || skillId == RETURN_TO_NATURE
			...
			int bonus = Math.max(0, (_attacker.getLevel() - Math.max(60, _target.getLevel())) / 4);
	...
	private int calcNpcMagicDamage(int skillId) {
		...
		if(_calcType == NPC_NPC) {
			if(_attacker instanceof L1PetInstance) {
				if (L1Attack.isImmune(_target) || isMissingSkillEffect((L1PetInstance)_attacker)) {
					dmg = 0;
				}
			}
		} else {
